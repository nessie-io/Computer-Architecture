#linear search
#int linearSearch(int arr[], int n, int target) {
#   for (int i = 0; i < n; i++) {
#        iif (arr[i] == target) return i; // found it, return the index
#   }
#    return -1; // not found
#}

main:
    #array in x30 and len in x11
    # Anchor the array to the stack pointer (sp) for guaranteed valid memory
    addi sp, sp, -32
    add x30, sp, zero    #(array)
    
    li x20, 6
    sw x20, 0(x30)
    li x20, 1
    sw x20, 4(x30)
    li x20, 5
    sw x20, 8(x30)
    li x20, 3
    sw x20, 12(x30)
    li x20, 2
    sw x20, 16(x30) # made an array with 5 numbers

    li x11, 5 # the length of the array (len)
    li x12, 3 # target

    jal x1, search

    # Syscall 93 exits the program and prints "Exited with error code [x10]"
    li x17, 93
    ecall  #code for ending the run

search:
    beq x30, zero, end  #if array == NULL
    beq x11, zero, end  #if  len == 0

    li x9, 0 #initializing i
    addi x23, x0, -1 #initializing default return value (-1 for not found)

    for:
    bge x9, x11, endloop

    slli x5, x9, 2
    add x5, x30, x5  #calculating base address
    lw x21, 0(x5) #storing the value in array position a[i]

    beq x12, x21, pass #target found 

    addi x9, x9, 1  #incrementing 
    j for

    pass:
    addi x23, x0, 0   #updating return value to 0 (found)

    endloop: 
    add x10, x0, x23   #move our result into x10
    
    end: jalr x0, 0(x1)